...
class MyDB_BufferManager {
  ...
  public:
    ...
    // every time this is called, look for the storage location and pg num
    // if in the map, return a share pointer
    // if not, make a sharepointer and store in the map.
    ...
    // buffer memory. since it is just a temp page, it is not associated with
  ...
  private:
    // actual storage pool
    char *bufferPool;
    // Clock Algo with other stuff
    MyDB_Clock clock;
    // where to store temporary pages
    string tempFile;
};
