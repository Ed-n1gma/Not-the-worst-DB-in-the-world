...
function <bool ()> buildRecordComparator (MyDB_RecordPtr lhs,  MyDB_RecordPtr rhs, const string& leftComp, const string& rightComp) {
	// compile a computation over the LHS and over the RHS
	char *str = (char *) leftComp.c_str ();
	pair <func, MyDB_AttTypePtr> lhsFunc = lhs->compileHelper (str);

	str = (char *) rightComp.c_str ();
	pair <func, MyDB_AttTypePtr> rhsFunc = rhs->compileHelper (str);
