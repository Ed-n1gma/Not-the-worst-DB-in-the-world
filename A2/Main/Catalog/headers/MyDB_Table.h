...
class MyDB_Table {
  ...
  public:
    ...
    // creates a table with the given name, at the given storage location, and
    // the given schema
    MyDB_Table(string tableName, string storageLoc, MyDB_SchemaPtr mySchema);

    // creates a table with the given name, at the given storage location, and
    // the given schema, of the given type, with the given sort att
    MyDB_Table(string tableName, string storageLoc, MyDB_SchemaPtr mySchema,
               string fileType, string sortAtt);

    // creates an empty table object
    MyDB_Table();

    // load the table information from the catalog; return false iff not in the
    // catalog
  ...
  private:
    ...
    // the last used page in the table, int for pagenum of a file
    int last;
