...
class MyDB_AttVal {
  ...
  public:
    ...
    virtual void serialize(char *&buffer, size_t &allocatedSize,
                           size_t &totSize) = 0;
    virtual ~MyDB_AttVal();

    // this gets a pointer to our data... useful because we can avoid
    // deserializing the record
    inline void *getDataPointer() { return myData; }

    inline void extendBuffer(char *&buffer, size_t &allocatedSize,
                             size_t &totSize, int extraSpaceNeeded) {
    ...
    MyDB_AttVal() { setNotBuffered(); }

    // means there's a size (type of short) for every "guy"
...
class MyDB_IntAttVal : public MyDB_AttVal {
  ...
  public:
    ...
    void serialize(char *&buffer, size_t &allocatedSize,
                   size_t &totSize) override;
    void set(int val);
    MyDB_IntAttVal();
    ~MyDB_IntAttVal();
