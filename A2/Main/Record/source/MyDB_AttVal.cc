...
int MyDB_IntAttVal ::toInt() {
    ...
    // because its been changed by fromInt(), so the dataptr should be invalid
    if (dataPtr == nullptr)
        return value;
    else
        return *((int *)dataPtr);
...
size_t MyDB_IntAttVal ::hash() { return std ::hash<int>()(toInt()); }

size_t MyDB_DoubleAttVal ::hash() { return std ::hash<int>()(toDouble()); }

size_t MyDB_BoolAttVal ::hash() { return std ::hash<int>()(toBool()); }

size_t MyDB_StringAttVal ::hash() { return std ::hash<string>()(toString()); }
...
// it seems that this funciton is to append its data (with its size at front) to
// whatever buffer is definitly not bufferpool because we may extend
// (new&delete) this buffer
// turns out its the buffer in MyDB_Record
void MyDB_IntAttVal ::serialize(char *&buffer, size_t &allocatedSize,
                                size_t &totSize) {
...
void MyDB_DoubleAttVal ::serialize(char *&buffer, size_t &allocatedSize,
                                   size_t &totSize) {

    extendBuffer(buffer, allocatedSize, totSize,
...
void MyDB_StringAttVal ::serialize(char *&buffer, size_t &allocatedSize,
                                   size_t &totSize) {

    string value = toString();

    extendBuffer(buffer, allocatedSize, totSize,
                 strlen(value.c_str()) + 1 + sizeof(short));

    *((short *)(buffer + totSize)) =
        (short)(sizeof(short) + strlen(value.c_str()) + 1);
    totSize += sizeof(short);
...
void MyDB_BoolAttVal ::serialize(char *&buffer, size_t &allocatedSize,
                                 size_t &totSize) {
